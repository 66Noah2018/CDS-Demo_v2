...
async function testMeasurementEndpoint() {
    ...
    request.get('http://localhost:3000/measurements/17', function(err, response, body) {
        if (err) throw err;
        if (response.statusCode == 200) {
            console.log('Get all measurements for patient 17: OK');
        } else {
            console.error('ERROR. Get all measurements for patient 17 failed with statuscode ' + response.statusCode);
        }
    })
    ...
    request.get('http://localhost:3000/measurements/17/17', function(err, response, body) {
        if (err) throw err;
        if (response.statusCode == 200) {
            console.log('Get measurement 17: OK');
        } else {
            console.error('ERROR. Get measurement 17 failed with statuscode ' + response.statusCode);
        }
    })
